...
> 由非营利性组织 `OpenAI Incorporated` 组成
> 及其盈利子公司 `OpenAI Limited Partnership`。
...
```
有关 `Azure` 包装器的更详细说明，请参见 [这里](/docs/integrations/llms/azure_openai)。
...
您可以在多个地方使用`tiktoken`分词器。默认情况下，它用于计算OpenAI大型语言模型的令牌。
用于OpenAI大型语言模型。
...
```
有关此内容的更详细说明，请参见[此笔记本](/docs/how_to/split_by_token/#tiktoken)。
